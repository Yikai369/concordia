"""Unit tests for Impression Management PE components."""
...
class MockLanguageModel(language_model.LanguageModel):
  ...
  def sample_text(
    ...
    # Simple mock: return a number for numeric requests, dialogue for dialogue requests
    if 'number' in prompt.lower() or 'rate' in prompt.lower() or 'estimate' in prompt.lower():
      return '0.75'
    if 'DIALOGUE' in prompt:
      return 'DIALOGUE: This is a test response.\nBODY: Nodding confidently'
    if 'reflection' in prompt.lower():
      return 'I will try to be more clear and direct.'
...
class IMPEMemoryComponentTest(unittest.TestCase):
  ...
  def test_add_evaluation_record(self):
    ...
    goal = impe.Goal(name='test', description='test goal')
    memory = impe.IMPEMemoryComponent(goal=goal)
    utterance = impe.Utterance(turn=1, speaker='Jane', text='Hello', body='smiling')
