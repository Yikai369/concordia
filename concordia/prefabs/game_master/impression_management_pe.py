...
"""A prefab game master for Impression Management PE conversation."""
...
class GameMaster(prefab_lib.Prefab):
  """A prefab game master for asymmetric IMPE conversation.

  The actor always speaks first, then the audience evaluates and responds.
  This creates an asymmetric turn structure.
  """

  description: str = (
      'A game master for Impression Management PE conversation. '
      'Handles asymmetric turn structure: actor acts first, audience evaluates and responds.'
  )
  ...
  def build(
    ...
    """Build a game master for IMPE conversation.
    ...
    """
    name = self.params.get('name', 'IMPE Conversation Rules')
    next_game_master_name = self.params.get('next_game_master_name', 'default rules')
    can_terminate_simulation = self.params.get('can_terminate_simulation', True)
    actor_name = self.params.get('actor_name', 'John')
    audience_name = self.params.get('audience_name', 'Jane')
    ...
    send_events_to_players_key = '__send_events_to_players__'
    ...
    # Fixed order: actor always acts first
    ...
    next_actor = gm_components.next_acting.NextActingInFixedOrder(
        sequence=[actor_name, audience_name],  # Actor first, then audience
    )
